...
- 2026-09-23：仅视觉首页使用Cover Flow，中央横向封面放大至桌面64vw（最高1040px、受视口高度约束）、手机88vw，两侧透视后退，位置随视口外扩并超出左右边缘裁切，避免缩小后露出边距；390/1440/1920/2560px侧卡外缘覆盖屏幕两端且无横向溢出，桌面截图已检查。默认黑白压暗，并叠加朝内侧由透明至55%黑的渐变（左卡向右加深，右卡向左加深），hover/键盘聚焦淡出渐变并恢复彩色，选中居中保持彩色；复用0.7秒滤镜过渡。中央卡默认1.025、两侧额外缩至0.9，拉开尺寸层级；中央悬停1.06、按下回缩至1（0.12秒），显式固定transform-origin为50% 50%，保持中心与详情跳转；浏览器实测缩放比例、中心位置及跳转通过。移除姓名岗位、顶部英文标注、左右箭头、说明文案及查看项目链接；吃豆人幻灯片与下方项目缩略图统一使用详情首图cover.webp（下方原已使用），底部仅项目标题和切换指示点，中央封面仍可进入详情。SCROLL TO EXPLORE移入视觉Hero内并绝对定位于底边（42px高），仅保留一个提示，其他岗位位置不变；1440×900/390×844实测提示底边与首屏一致、不遮挡标题，截图已检查。恢复5.5秒自动切换，悬停/焦点/离屏/隐藏标签页暂停，减少动态效果不自动播放；保留侧卡选择、圆点/键盘、拖动及横向触控板。
- 共用岗位列表仅alljobs渲染，各岗位包括preview=1均不再显示；其他岗位Hero布局不变。相关partial/role-switcher.ejs、partial/visual-coverflow.ejs、css/visual-coverflow.css、js/visual-coverflow.js。构建、语法、1440/390px无溢出、删项检查、自动从雷朋切至巨量引擎、alljobs与五岗位隔离通过；桌面手机截图已检查。前轮侧卡/拖动/循环/跳转已验证。新增黑白/hover/选中三态浏览器计算样式检查及构建通过，纵深渐变构建及差异检查通过；最新尺寸层级1440/390px无溢出验证通过，两端截图已检查。预览4025，未提交或部署。

- 发布完成：f6b5bed 已提交并推送 main，GitHub Pages 运行35842409556为completed/success。线上视觉首页已核实新分组、巨量引擎/吃货豆轮播及九州入口移除，巨量引擎详情两节内容已核实。下方本轮未部署记录由本次发布状态统一覆盖。
