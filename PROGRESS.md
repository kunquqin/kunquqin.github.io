...
根地址改为空白页，完整首页移至 `/alljobs/`，本轮修改已在本地完成，待部署。

## 当前实现

- 所有使用共用顶部导航的页面左侧标识统一为 Kevin，移除小字描述；构建通过，未做本轮视觉验证，待部署。
...
- 本轮构建通过；静态断言验证空白根页、完整首页标签、综合项目返回、AI 默认隐藏标签及六个岗位项目返回路径。未做本轮浏览器视觉验证，未提交、推送或部署。
