...
AI Native Builder 职业经历标题由“个人产品实践”改为“产品开发”；构建与生成页面文案检查通过，正在重新部署。

## 当前实现

- AI Native Builder 职业经历已按 `简历_ai产品设计师.png` 更新：产品开发、得物 C 端体验设计、哔哩哔哩资深美术设计、字节跳动资深创意设计、阿里/饿了么资深视觉设计，以及广告公司与自由设计经历；职责压缩为结果型简述。已提交并推送 eb143a4，GitHub Actions 34206153337 completed/success；线上页面检查通过。
...
- 综合岗位标签、共用导航及原项目的根地址返回链接改为 `/alljobs/`；自用岗位切换进入 AI 时携带 `?preview=1`，直接 AI 岗位地址保持隐藏标签。综合版/全部版本启动脚本同步更新。
